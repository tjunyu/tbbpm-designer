 ...
 *******************************************************************************/
package com.taobao.tbbpm.designer.editors.router;
 ...
 * @author Randy Hudson
 * @since 3.0 Modified by junyu.wy
 */
public class TBBPMPathRouter {
	...
	private void bendPaths() {
		for (int i = 0; i < orderedPaths.size(); i++) {
			...
			path.getPoints().addPoint(new Point(path.start.x, path.start.y));
			for (int v = 0; v < path.grownSegments.size(); v++) {
				...
				if (vertex != null && v < path.grownSegments.size() - 1) {
					if (vertex.type == Vertex.INNIE) {
						vertex.count++;
						path.getPoints().addPoint(vertex.bend(vertex.count));
					} else {
						path.getPoints().addPoint(
								vertex.bend(vertex.totalCount));
						vertex.totalCount--;
					}
				}
			}
			path.getPoints().addPoint(new Point(path.end.x, path.end.y));
		}
	}
		...
		for (int i = 0; i < workingPaths.size(); i++) {
			...
			if (path.grownSegments.size() == 0) {
				for (int s = 0; s < path.segments.size(); s++)
					testOffsetSegmentForIntersections(// 抵消间隙对于交叉点
	...
	public static void updateNewLocation(Point pre, Point next,
			Point newLocation) {
		if ((pre == null) || (next == null)) {
			return;
		}
		if (isNearRectangle(pre, next, newLocation))
			if (newLocation.getDistance2(new Point(pre.x, next.y)) < newLocation
					.getDistance2(new Point(next.x, pre.y))) {
				newLocation.setLocation(pre.x, next.y);
			} else
				newLocation.setLocation(next.x, pre.y);
	}

	public static boolean isNearRectangle(Point pre, Point next,
			Point newLocation) {
		if ((pre == null) || (next == null) || (newLocation == null)) {
			return false;
		}
		double dist1 = pre.getDistance2(newLocation)
				+ next.getDistance2(newLocation);
		double dist2 = pre.getDistance2(next);
		if (dist2 != 0.0D) {
			double r = Math.abs((dist1 - dist2) / dist2);
			if (r < 0.35D) {
				return true;
			}
		}

...
import com.taobao.tbbpm.define.impl.DecisionNode;
 ...
 * 
 */
public class DescisionElementNode implements CommNodeInterface {
	...
	public String checkout(INode abstractNode) {
		DecisionNode dNode = (DecisionNode) abstractNode;
		Action action = dNode.getAction();
		String target = CommNodeInterface.rightTarget;
		if (action != null) {
			ActionHandle handle = (ActionHandle) action.getActionHandle();
			if (dNode.getAction().getType().length() == 0) {
				dNode.setAction(null);
